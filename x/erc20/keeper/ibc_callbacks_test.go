	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() transfertypes.FungibleTokenPacketData {
				...
				_, err = suite.network.App.EVMKeeper.CallEVM(ctx, contracts.ERC20MinterBurnerDecimalsContract.ABI, suite.keyring.GetAddr(0), contractAddr, true, "mint", nil, types.ModuleAddress, big.NewInt(10))
				suite.Require().NoError(err)
